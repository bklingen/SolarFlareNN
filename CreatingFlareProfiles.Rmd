...
cutoff = 0.005 #0.00270
Feb23Flares1 = Feb23Flares1 %>% mutate(yhat = 1*(predProb > cutoff), predProb = round(predProb,4),
                                       agree = factor(10*Flare + yhat, levels = c(0,1,10,11), labels=c('nf,nf', 'nf,f', 'f,nf', 'f,f')))
#head(Feb23Flares1)
...
Feb23Fitted = Feb23Flares1 %>% select(-Date, -agree) %>% mutate(across(everything(), ~ c(.x)))
