...
NotFlareProfiles = matrix(NA, nrow=50*numFlares, ncol=hist1 + hist2 + 1 + 1 + 1)
nonflare_indices = setdiff( #rule out those within 3 minutes of peak flare
  c((hist1 + 1) : (nrow(Flares2021)- (hist2 + 1))), 
  c(flare_indices, 
    flare_indices - 1, flare_indices - 2, flare_indices - 3,
    flare_indices + 1, flare_indices + 2, flare_indices + 3
    )
)
j = 1
for (i in sample(nonflare_indices, 50*numFlares)){ #select random index
